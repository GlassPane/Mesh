 ...
 */
package dev.upcraft.mesh.impl.resource;
...
public class CraftingVirtualResourcePack implements ResourcePack {
    ...
    public InputStream open(ResourceType type, Identifier name) throws IOException {
        if (type == ResourceType.SERVER_DATA) {
            if (DATA_CACHE.containsKey(name)) {
                return new ByteArrayInputStream(JsonUtil.GSON.get().toJson(DATA_CACHE.get(name)).getBytes(StandardCharsets.UTF_8));
            }
        }
    ...
    public <T> T parseMetadata(ResourceMetadataReader<T> reader) throws IOException {
        ...
        meta.add("pack", pack);
        try (InputStream inputStream = new ByteArrayInputStream(JsonUtil.GSON.get().toJson(meta).getBytes(StandardCharsets.UTF_8))) {
