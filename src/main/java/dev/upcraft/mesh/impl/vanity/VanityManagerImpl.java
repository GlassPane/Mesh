...
import dev.upcraft.mesh.impl.registry.MeshRegistries;
    ...
    public CompletableFuture<Void> parseRemoteConfig(String url) {
        return MeshApiOptions.VANITY_FEATURES_ENABLED ? CompletableFuture.supplyAsync(() -> {
            ...
            try (InputStreamReader input = new InputStreamReader(new URL(url).openStream())) {
                JsonObject json = JsonHelper.deserialize(JsonUtil.GSON.get(), input, JsonObject.class);
                if (json == null) {
            ...
            try {
                Map<Identifier, JsonObject> generalConfig = JsonUtil.GSON.get().fromJson(JsonHelper.getObject(json, "general", new JsonObject()), GENERAL_CONFIG_TOKEN);
                ...
                Map<UUID, Map<Identifier, JsonElement>> userConfig = JsonUtil.GSON.get().fromJson(JsonHelper.getObject(json, "users", new JsonObject()), USER_CONFIG_TOKEN);
