...
public class HandCommand {
    ...
    private static int getItemInHand(CommandContext<ServerCommandSource> context, Hand hand) throws CommandSyntaxException {
        ...
        if(!stack.isEmpty()) {
            Text result = new TranslatableText("command.mesh.debug.helditem_name", stack.getName());
            ...
            if(stack.hasTag()) {
                result = result.append("\n")
                        .append(new TranslatableText("command.mesh.debug.helditem_nbt", stack.getTag().toText()).setStyle(new Style()
